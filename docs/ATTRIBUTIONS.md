...
- Used for: the plain per-chapter recap shown at the top of the Context tab (`chapter_recap` table, `source = 'bible-summary'`). The summary text is kept locally in `build/data/recaps-biblesummary.json` (refresh with `build/fetch-biblesummary.mjs`; the normal build does not hit the network). The app attributes biblesummary.info in the recap card.
